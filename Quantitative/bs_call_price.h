...
auto bs_price_call_t(T1 S, T2 K, T3 r, T4 q, T5 sigma, T6 Tmat)
{
    using result_t = std::common_type_t<T1,T2,T3,T4,T5,T6>;
    using std::exp;
    using std::log;
    using std::sqrt;

    result_t F = S * exp((r - q) * Tmat);
    result_t sigma_sqrtT = sigma * sqrt(Tmat);;

    if (sigma_sqrtT == result_t(0)) {
        result_t payoff = (std::real(F) > std::real(K)) ? (F - K) : result_t(0);
        return exp(-r * Tmat) * payoff;
    }

    result_t d1 = (log(F / K) + 0.5 * sigma * sigma * Tmat) / sigma_sqrtT;
    result_t d2 = d1 - sigma_sqrtT;

    return exp(-r * Tmat) * (F * Phi_t(d1) - K * Phi_t(d2));
}